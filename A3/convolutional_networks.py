...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    x_clone = x.clone()
    x_pad = torch.nn.functional.pad(x_clone, (pad, pad, pad, pad))  #后两个参数默认为‘constant'和0
    
    out = torch.zeros(N, F, out_H, out_W, device = x.device, dtype = x.dtype)  #数据类型与输入保持一致，得到的误差更小   
        ...
        for f in range(F):
            ...
            for out_h in range(out_H):
                ...
                for out_w in range(out_W):
                    out[n, f, out_h, out_w] = torch.sum(x_pad[n, :, height:height+HH, width:width+WW] * w[f]) + b[f]
                    width += stride
                height += stride
                #输出的行列与输入的行列存在一定的关系，height和width可以用out_h * stride和out_w * stride代替
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b, conv_param)
  ...
  def backward(dout, cache):
    ...
    for n in range(N):
        for f in range(F):
            for out_h in range(out_H):
                for out_w in range(out_W):
                    dw[f] += dout[n, f, out_h, out_w] * \
                            x_pad[n, :, out_h*stride: out_h*stride+HH, out_w*stride: out_w*stride+WW]
                    dx[n, :, out_h*stride: out_h*stride+HH, out_w*stride: out_w*stride+WW] += \
                            dout[n, f, out_h, out_w] * w[f]  #这里用dout矩阵中的某个元素与w[f]相乘是因为L=g(out1,……,outn)，各个out又是w[f]与输入局部数据卷积的结果
                    db[f] += dout[n, f, out_h, out_w] 
    ...
    for n in range(N):
        for c in range(C):
            for out_h in range(out_H):
                for out_w in range(out_W):
                    out[n, c, out_h, out_w] = \
                        torch.max(x[n, c, out_h*stride:out_h*stride+pool_height, out_w*stride:out_w*stride+pool_width])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
  ...
  def backward(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for out_h in range(out_H):
                for out_w in range(out_W):
                    local = torch.zeros(pool_height * pool_width, dtype = x.dtype, device = x.device)
                    idx = torch.argmax(x[n, c, out_h*stride:out_h*stride+pool_height, out_w*stride:out_w*stride+pool_width])
                    local[idx] = 1
                    local = local.reshape(pool_height, pool_width)
                    dx[n, c, out_h*stride:out_h*stride+pool_height, out_w*stride:out_w*stride+pool_width] = \
                        dout[n, c, out_h, out_w] * local
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    ############################################################################
    # Replace "pass" statement with your code
    F, C, f_size = num_filters, input_dims[0], filter_size  #input_dims是元组，索引数据直接[index]
    in_size = input_dims[1]
    conv_out_size = in_size  #计算数据见loss()里的conv_param
    pool_out_size = in_size//2  #  #计算数据见loss()里的pool_param
    self.params['W1'] = weight_scale * torch.randn(F, C, f_size, f_size, dtype = dtype, device = device)
    self.params['b1'] = torch.zeros(F, dtype = dtype, device = device)
    self.params['W2'] = weight_scale * torch.randn(pool_out_size**2*F, hidden_dim, dtype = dtype, device = device)  #这里16为conv_pool_out_size，该变量名过长，故直接使用16替代
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    hidden_l1, cache_l1 = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
    #hidden_l1 = hidden_l1.reshape(-1)  #不用自己展成一维数据，class Linear里已经做了展开处理
    hidden_l2, cache_l2 = Linear_ReLU.forward(hidden_l1, W2, b2)
    scores, cache_l3 = Linear.forward(hidden_l2, W3, b3)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    #loss += self.reg * (torch.sum(W1 * W1 + W2 * W2 + W3 * W3))  #不可先累加后sum，因为各层权重形状不一致无法累加
    loss += self.reg * (torch.sum(W1*W1)+torch.sum(W2*W2)+torch.sum(W3*W3))  #L2正则项
    #求梯度
    dhidden_l2, grads['W3'], grads['b3'] = Linear.backward(dscores, cache_l3)
    dhidden_l1, grads['W2'], grads['b2'] = Linear_ReLU.backward(dhidden_l2, cache_l2)
    _, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dhidden_l1, cache_l1)
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  model = DeepConvNet(input_dims = data_dict['X_train'].shape[1:], num_classes = 10,
                      num_filters = [32, 64, 128], max_pools = [1, 2],
                      weight_scale = 'kaiming', reg = 1e-5, dtype = dtype, device = device)
  solver = Solver(model, data_dict, num_epochs = 5, batch_size = 128, update_rule = adam,
                  optim_config={
                    'learning_rate': 2e-3,
                  },
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
  ...
  if K is None:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    weight = torch.sqrt(torch.tensor(gain/(Din*K*K))) * torch.randn(Dout, Din, K, K, dtype = dtype, device = device)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
  return weight
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    """
    mode = bn_param['mode']
    eps = bn_param.get('eps', 1e-5)  #返回指定键的值，若不存在则返回默认值
    ...
    if mode == 'train':
      ...
      #var = torch.sum(torch.pow(x - mean, 2), dim = 0) / N  #(D,)
      var = torch.var(x, dim=0, unbiased=False)
      norm = (x - mean) / ((var + eps) ** 0.5)  #(N, D)
      out = gamma * norm + beta
      running_mean = momentum * running_mean + (1 - momentum) * mean
      running_var = momentum * running_var + (1 - momentum) * var
      cache = (x, bn_param, mean, var, norm, gamma)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    elif mode == 'test':
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    x, bn_param, mean, var, norm, gamma = cache
    N, _ = x.shape
    mode, eps = bn_param['mode'], bn_param.get('eps', 1e-5)
    #momentum = bn_param.get('momentum', 0.9)
    x_minus_mean = x - mean
    if mode == 'train':
        dbeta = dout.sum(dim=0)  #(D,)
        dgamma = (dout * norm).sum(dim=0)  #(D,)
        dnorm = dout * gamma  #(N, D)
        dvar = (dnorm * x_minus_mean).sum(dim=0) * (-1/2) * (var + eps).pow(-3/2)  #(D,)
        dx_minus_mean = torch.ones_like(x_minus_mean)  #(N, D)
        dx_minus_mean *= dnorm * (var + eps).pow(-1/2)
        dx_minus_mean += dvar * 2 * x_minus_mean / N
        dmean = -1 * dx_minus_mean.sum(dim=0)  #(D,)
        dx = torch.ones_like(x)  #(N, D)
        dx *= dmean / N
        dx += dx_minus_mean
    else:
        dbeta = dout.sum(dim=0)  #(D,)
        dgamma = (dout * norm).sum(dim=0)  #(D,)
        dx = gamma / (var + eps).sqrt()  #(N, D)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
  ...
  def backward_alt(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    #请参考：https://zhuanlan.zhihu.com/p/32995244
    x, bn_param, mean, var, norm, gamma = cache
    N, _ = x.shape
    ...
    dbeta = dout.sum(dim=0)  #(D,)
    dgamma = (dout * norm).sum(dim=0)  #(D,)
    dx = (dout - dgamma * norm / N - dout.mean(dim=0)) * gamma / std  #(N, D)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    #This is my code. It is not satisfied with the requirement that the code must less than 5 lines.
    #N, C, H, W = x.shape
    #x_flatten = x.reshape(N, -1)  #(N, D)
    #gamma_extend, beta_extend = torch.ones_like(x.reshape(N, C, -1)[0]), torch.ones_like(x.reshape(N, C, -1)[0])
    #gamma_extend = (gamma.reshape(-1, 1) * gamma_extend).reshape(-1)
    #beta_extend = (beta.reshape(-1, 1) * beta_extend).reshape(-1)
    #out, cache = BatchNorm.forward(x_flatten, gamma_extend, beta_extend, bn_param)
    #out = out.reshape(N, C, H, W)
    #This is others code which is satisfied the requirement.
    N, C, H, W = x.shape
  ...
  def backward(dout, cache):
    ...
    #This is my code. It is not satisfied with the requirement that the code must less than 5 lines.
    #N, C, H, W = dout.shape
    #dout_flatten = dout.reshape(N, -1)
    #dx, dgamma_extend, dbeta_extend = BatchNorm.backward_alt(dout_flatten, cache)
    #dx = dx.reshape(N, C, H, W)
    #dgamma_extend, dbeta_extend = dgamma_extend.reshape(C, -1), dbeta_extend.reshape(C, -1)
    #dgamma, dbeta = dgamma_extend.sum(dim=1), dbeta_extend.sum(dim=1)  
    #用以上方法计算得出的dbeta在执行Convolutional nets with batch normalization部分的第一个cell代码后，reg=3.14时的grad[bias]不是规定误差
    #This is others code which is satisfied the requirement.
    N, C, H, W = dout.shape
    dx = dout.permute(0, 2, 3, 1).reshape(-1, C)
    dx, dgamma, dbeta = BatchNorm.backward_alt(dx, cache)
