  ...
  for i in range(num_train):
    ...
    for j in range(num_classes):
      ...
      if margin > 0:
        ...
        #pass
        dW[:, j] += X[i]  #可以实现用行向量给列向量赋值,因为都是向量（1维）
        dW[:, y[i]] += -X[i]
        #######################################################################
        #                       END OF YOUR CODE                              #
        #######################################################################
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  idx_col = y
  correct_class_score = scores[idx_row, idx_col] #取出各样本实际类别得分并保存
  margin = scores - correct_class_score.view(-1, 1) + 1 #进行损失计算，这里要使用view()转换成列向量才可进行广播
  margin[idx_row, idx_col] = 0  #执行上一行对实际类别得分也进行了运算，而公式中不对实际类别得分作处理，故置0，方便后面取大于0的值
  loss = torch.sum(margin[margin > 0])/num_train + reg*torch.sum(W*W)  #bool索引取出大于0的值，别忘了除以num_train
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  ...
  mask[margin > 0] = 1
  error_class_num = torch.sum(mask, dim = 1)  #计算每个样本（每行）为损失做出贡献的错误类别数量
  mask[idx_row, idx_col] = -error_class_num  
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [3e-3, 5e-3, 7e-3, 1e-2, 2e-2]
  regularization_strengths = [1e-1, 3e-1, 5e-1, 1e0, 5e0]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  # Replace "pass" statement with your code
  model = cls
  model.train(data_dict['X_train'], data_dict['y_train'],  #关于该函数需细究，尤其是类中的loss，这里W作为self.W
              learning_rate = lr, reg = reg, num_iters = 100)   #这里迭代了100次，如果设置成2000的话时间有点久
  y_train_pred = model.predict(data_dict['X_train'])
  y_val_pred = model.predict(data_dict['X_val'])
  train_acc = 100*torch.mean((y_train_pred == data_dict['y_train']).double())
  val_acc = 100*torch.mean((y_val_pred == data_dict['y_val']).double())
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
      ...
      scores_exp_sum = scores.exp().sum()
      probability = scores_exp/scores_exp_sum
      loss += -torch.log(probability[y[i]])
      #求梯度
      for j in range(num_classes):
          if j == y[i]:
              dW[:, j] += (scores_exp[y[i]]/scores_exp_sum-1)*X[i]
          else:
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores = scores - m.view(-1, 1)
  scores_exp = scores.exp()
  scores_exp_sum_row = torch.sum(scores_exp, dim = 1)
  probability = scores_exp.div(scores_exp_sum_row.view(-1, 1))
  loss = torch.sum(-torch.log(probability[torch.arange(num_train), y]))/num_train + reg*torch.sum(W*W)
    
  mask = probability.clone()  # = scores_exp.div(scores_exp_sum_row.view(-1, 1))
  mask[torch.arange(num_train), y] = scores_exp[torch.arange(num_train), y]/scores_exp_sum_row - 1
  dW = X.t().mm(mask)/num_train + 2*reg*W
  #############################################################################
  #                          END OF YOUR CODE                                 #
  #############################################################################
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1.2e-1, 1.25e-1, 1.3e-1, 1.35e-1, 1.4e-1]
  regularization_strengths = [5e-4, 7e-4, 9e-4, 1e-3]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
